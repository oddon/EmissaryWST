...
$(document).ready(function($){
    $('.my-form:last .add-box').click(function(){
        ...
        if (MAX_NUMBER_OF_FIELDS - 1 < n) {
            alert('Max number of fields that can be added is ' + MAX_NUMBER_OF_FIELDS);
            return false;
        }
        var box_html = $(`
            <p class="text-box">
                <label id = "added_label" for="optional_' + n + '"> 
                    <span class="box-number">' + label + '</span>
                </label>
                <br> 
                <input type="text" name="boxes[]" value="" placeholder = "Enter here" id="box' + n + '" required/> 
